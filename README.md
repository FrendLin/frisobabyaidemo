...
- **选择目录**：默认使用系统目录选择器；由于浏览器安全限制无法回传绝对路径，选择后需在输入框补全完整路径，或直接粘贴目录路径。服务在本机读取该目录（递归扫描，忽略非图片），因此适合本地/内网使用。
- **标注**：每张图片只标一个主要、明显的品牌和一个物料类型。
  - 品牌预置：皇家美素（皇家、旺玥、源悦、尊悦）、常见品牌（爱他美、飞鹤、金领冠、启赋、a2、雀巢、美赞臣、君乐宝、合生元）、辅助（其他、无法判断）。
  - 物料类型严格采用审核标准六类：灯箱、橱窗单透/墙贴、店内海报（非货架）、店招/外立面广告、吊旗、包柱画面。
- **自动保存**：品牌与物料选定后自动保存，另保留“保存并下一张”；界面显示保存中 / 已保存 / 失败可重试。
...
- 只有**皇家四子品牌 + 六类物料 + 已完成**的标注才会写回（竞品、其他、无法判断、未标完的图片会被跳过并计数），因为清单服务于皇家子品牌识别；
