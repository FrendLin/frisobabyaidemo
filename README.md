...
- 单张图片审核：品牌与物料类型均为可选条件；任一有值进入审核模式（按所选维度比对），两项都不选进入纯识别模式（返回一张图中的多个「品牌 · 物料」组合，纯识别不产生“审核通过”结论）；
- 多组合识别：模型可返回同图多个可归属载体（如“皇家 · 灯箱、尊悦 · 灯箱”），按“品牌 + 类型”去重并限制最大结果数，同时保留旧 `detected_brand` / `detected_material_type` 作为“主结果”兼容字段；
- 七类物料类型（灯箱、橱窗单透/墙贴、店内海报〔非货架〕、店招/外立面广告、吊旗、包柱画面、嵌柜）；
- Excel 批量审核：读取“品牌 / 物料类型 / 图片链接”（批处理仍要求两项必填），保留原字段并追加结果；
- 低置信度人工复核、画质预警、识别证据与失败关闭；
- 220 张历史基线的固定 176/44 分层划分及 90% 联合准确率门禁脚本；本轮补充数据以版本化独立清单接入（见下）；
...
默认 seed 为 `20260724`，总样本 220，训练 176，测试 44。固定清单已提交到 `data/dataset_manifest.csv`，作为历史基线保持不变、可比。

本轮客户补充的 152 条 URL 样本以**版本化独立清单**接入，不重洗历史测试集：

```bash
python scripts/prepare_supplement.py "/path/to/品牌物料补充训练0727.xlsx" \
    --version 20260727 \
    --existing-manifest data/dataset_manifest.csv \
    --output data/dataset_supplement_20260727.csv
```

脚本完成去重（含与既有清单排重）、品牌合法性校验、“包柱 → 包柱画面”规范化，并按品牌、物料类型、联合标签打印统计与非法/缺失行报告。数据版本、7 张本地嵌柜图逐张处理状态及旺玥/尊悦缺样说明见 [`docs/data-version-20260727.md`](docs/data-version-20260727.md)。
...
- **标注**：每张图片可选择一个或多个品牌，以及一个或多个物料类型。
  ...
  - 物料类型严格采用审核标准七类：灯箱、橱窗单透/墙贴、店内海报（非货架）、店招/外立面广告、吊旗、包柱画面、嵌柜。
...
- 只有**恰好一个皇家子品牌 + 恰好一个标准物料类型 + 已完成**的标注才会写回，因为现有训练清单是单标签结构；多品牌/多物料、竞品、其他、无法判断、未标完的图片会分别跳过并计数，不会被错误压缩成单标签；
